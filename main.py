...
async def start_command(message: types.Message, state: FSMContext):
	...
	if db.get_users_exist(message.chat.id) == False:
		...
		if not args:
			await bot.send_message(chat_id=message.chat.id, text='📨 Привет, это бот файлообменник\n💎 Наши боты:\n@yanetix_shop_bot - Наш магазин\n@yanetix_vpn_bot - Наш VPN-бот\n@yanetix_gpt_bot - Наш чат-бот GPT', reply_markup = kb.menu_kb())
		else:
			...
			if type_file is None and fileID is None:
				...
				if password == (None,):
					...
					await bot.send_message(chat_id=message.chat.id, text='⚠️ Введите пароль:', reply_markup = kb.back_kb())
	...
	if message.text:
		if message.text.lower() == 'отмена':
			...
			if message.text == password[0]:
				...
				await bot.send_message(chat_id=message.chat.id, text='❌ Пароль неверный. Попробуйте еще раз:', reply_markup = kb.back_kb())
	else:
		await bot.send_message(chat_id=message.chat.id, text='❌ Пароль неверный. Попробуйте еще раз:', reply_markup = kb.back_kb())
...
async def create_post(message: types.Message):
	if db.get_users_exist(message.chat.id) == True:
		await bot.send_message(chat_id=message.chat.id, text='📃 Отправьте мне файл.', reply_markup = kb.back_kb())
...
async def create_post(message: types.Message):
	if db.get_users_exist(message.chat.id) == True:
		...
		if all_types == []:
			await bot.send_message(chat_id=message.chat.id, text='⚠️ У вас нету загруженных файлов, чтобы загрузить файлы нажмите "Загрузить файл"', reply_markup = kb.menu_kb())
		else:
			text='Ваши файлы: \n\n'
...
async def upload_file(message: types.Message, state: FSMContext):
	...
	if message.photo:
		...
		await bot.send_message(chat_id=message.chat.id, text='⚠️ Введите пароль. Если не нужен, пишите "-"', reply_markup=kb.back_kb())
		await Info.upload_file_password.set()
	elif message.text:
		if message.text.lower() == 'отмена':
			await bot.send_message(chat_id=message.chat.id, text='Вы вернулись в назад.', reply_markup=kb.menu_kb())
			await state.finish()
		else:
			await bot.send_message(chat_id=message.chat.id, text='📃 Отправьте мне файл.', reply_markup=kb.back_kb())
	elif message.voice:
